...
// the given key).
func (validator *Validator) AddError(key string, message string) {
	if _, exist := validator.Errors[key]; !exist {
		validator.Errors[key] = message
	}
}

// Check adds an error message to the map only if a validation check is not 'ok'.
func (validator *Validator) Check(ok bool, key string, message string) {
	if !ok {
